...
class DamDecoder:
    ...
    def decode_float_array(self):
        ...
        num = self.read_int64()
        # Fast path: bulk unpack from buffer directly using struct.unpack_from,
        # avoiding a Python loop and per-element assignment
        floats = struct.unpack_from(f"{num}d", self.buffer, self.pos)
        self.pos += num * 8
        return list(floats)
