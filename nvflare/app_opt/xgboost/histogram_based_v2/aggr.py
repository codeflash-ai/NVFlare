...
class Aggregator:
    ...
    def aggregate(self, gh_values: list, sample_bin_assignment, num_bins, sample_ids):
        ...
        # Optimize by moving common indexable var access and looping out of function call
        if not sample_ids:
            gh_len = len(gh_values)
            for sample_id in range(gh_len):
                bin_id = sample_bin_assignment[sample_id]
                if bin_id < 0:
                    continue
                ...
                if current_value == 0:
                    ...
                    aggr_result[bin_id] = self.add(current_value, sample_value)
        else:
