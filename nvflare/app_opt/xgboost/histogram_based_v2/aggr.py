...
class Aggregator:
    ...
    def aggregate(self, gh_values: list, sample_bin_assignment, num_bins, sample_ids):
        ...
        if not sample_ids:
            # Inlined version of _update_aggregation for performance
            add = self.add if hasattr(self, "add") else None  # avoid repeated lookup
            ...
            add = self.add if hasattr(self, "add") else None
