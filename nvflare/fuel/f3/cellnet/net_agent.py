...
class NetAgent:
    def __init__(self, cell, change_root_cb=None, agent_closed_cb=None):
        ...
        # Register all cell callbacks in a single loop for performance and readability
        cb_topic_pairs = [
            (self._do_report_cells, _TOPIC_CELLS),
            (self._do_route, _TOPIC_ROUTE),
            (self._do_start_route, _TOPIC_START_ROUTE),
            (self._do_stop, _TOPIC_STOP),
            (self._do_stop_cell, _TOPIC_STOP_CELL),
            (self._do_peers, _TOPIC_PEERS),
            (self._do_connectors, _TOPIC_CONNS),
            (self._do_url_use, _TOPIC_URL_USE),
            (self._do_speed, _TOPIC_SPEED),
            (self._do_echo, _TOPIC_ECHO),
            (self._do_stress, _TOPIC_STRESS),
            (self._do_change_root, _TOPIC_CHANGE_ROOT),
            (self._do_bulk_test, _TOPIC_BULK_TEST),
            (self._do_bulk_item, _TOPIC_BULK_ITEM),
            (self._do_msg_stats, _TOPIC_MSG_STATS),
            (self._do_list_pools, _TOPIC_LIST_POOLS),
            (self._do_show_pool, _TOPIC_SHOW_POOL),
            (self._do_comm_config, _TOPIC_COMM_CONFIG),
            (self._do_config_vars, _TOPIC_CONFIG_VARS),
            (self._do_process_info, _TOPIC_PROCESS_INFO),
            (self._do_heartbeat, _TOPIC_HEARTBEAT),
        ]
        for cb, topic in cb_topic_pairs:
    ...
    def _do_comm_config(self, request: Message) -> Union[None, Message]:
        # Optimize Message payload creation: avoid keyword passing for single argument
